...
- [ ] **Introduce Kafka for event-driven audit logging.** Today audit rows are written via synchronous REST. Add Kafka so producers (Auth on login/refresh, plus a sample domain event) publish to a topic and Audit consumes asynchronously to persist `AuditLog`s — decoupling the audit store from request latency and showing an event-driven architecture. Scope: Kafka (or Redpanda) in the docker-compose stack; `spring-kafka` producer in Auth (an `AuditEvent` record + `KafkaTemplate`); a `@KafkaListener` consumer in Audit; topic `audit.events` keyed by `entityType`; consumer retry + dead-letter topic; idempotent/at-least-once handling. Strong interview talking point (async messaging, delivery semantics, backpressure) and directly addresses the "generic CRUD" gap above.
