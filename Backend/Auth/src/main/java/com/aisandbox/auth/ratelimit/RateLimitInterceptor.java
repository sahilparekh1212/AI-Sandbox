 ...
 * request for the same key is discarded (cancelled). On {@code afterCompletion}
 * the request deregisters and the worker thread's interrupt flag is cleared so
 * it cannot leak into the next pooled request.
 */
@Component
